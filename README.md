...
- ✅ Track subsystems and message counts per source (engine, proxy, repository, scheduler, etc.)
- ✅ Automatic discovery notification when new subsystems appear
- ✅ Dynamic search term management (add/remove terms on-the-fly)
- ✅ Highlight messages matching search terms
- ✅ Text-based table display with color-coded output
...
```
╔═══════════════════════════════════════════╗
║   Qlik Sense UDP Log Scanner v1.0.0      ║
╚═══════════════════════════════════════════╝
...
├────────────────────┼──────────────────────────────────┼──────────┼────────────────┼─────────────────┤
│ qseow-engine       │ Engine.App                       │ 234      │ 192.168.1.10   │ reload          │
│ qseow-engine       │ Engine.Session                   │ 1        │ 192.168.1.10   │                 │
│ qseow-proxy        │ Proxy.Authentication             │ 892      │ 192.168.1.11   │ login, error    │
│ qseow-proxy        │ Proxy.Connection                 │ 445      │ 192.168.1.11   │                 │
│ qseow-scheduler    │ Scheduler.Task                   │ 67       │ 192.168.1.12   │ failed          │
└────────────────────┴──────────────────────────────────┴──────────┴────────────────┴─────────────────┘
...
│ Source             │ Subsystem                        │ Count    │ Sender IP      │ Search Matches  │
├────────────────────┼──────────────────────────────────┼──────────┼────────────────┼─────────────────┤
│ qseow-engine       │ Engine.App                       │ 456      │ 192.168.1.10   │ error, reload   │
│ qseow-engine       │ Engine.Session                   │ 1523     │ 192.168.1.10   │ timeout         │
│ qseow-proxy        │ Proxy.Authentication             │ 892      │ 192.168.1.11   │ login, error    │
│ qseow-proxy        │ Proxy.Connection                 │ 445      │ 192.168.1.11   │                 │
│ qseow-repository   │ Repository.Database              │ 234      │ 192.168.1.11   │                 │
│ qseow-scheduler    │ Scheduler.Task                   │ 156      │ 192.168.1.12   │ failed, error   │
└────────────────────┴──────────────────────────────────┴──────────┴────────────────┴─────────────────┘
...
│   ├── udp-server.js      # UDP server setup and message handling
│   ├── message-parser.js  # Parse UDP messages into structured objects
│   ├── stats-tracker.js   # Track subsystems, counts, and search matches
│   ├── search-manager.js  # Manage dynamic search terms
│   ├── display.js         # Text-based table display with colors
